...
8. Read `references/analysis-guide.md`. Read `<workspace>/data/profile/collaboration-profile.md` when present; otherwise use `references/collaboration-profile.md` only as a generic template. Write `<workspace>/reports/YYYY-MM-DD/owner--repo-v1.md`, targeting 8,000–12,000 Chinese characters unless explicitly overridden.
9. Add a tailored improvement section that maps transferable ideas to the current GitHub radar workflow. For each suggestion state context, proposed change, expected gain, cost/risk, priority, confidence, acceptance criteria, and `adopt / trial / defer / reject` recommendation. Keep user choice explicit.
